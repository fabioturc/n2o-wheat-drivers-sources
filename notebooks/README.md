...
- `DRIVERS_01_feature_selection.ipynb`  
  - Uses recursive feature elimination to define an importance ranking of features and identify the best feature set.  
  - Writes `best_features*` and `ranked_features*` to `../data/processed/`.

- `DRIVERS_02_hyperparameter_optimization.ipynb`  
  - Implements `RandomizedSearchCV` to find the optimal set of XGBoost hyperparameters.  
...
- `DRIVERS_10_shap_analysis.ipynb`  
  ...
  - Evaluates model performance on the last chronological 20% of the dataset (hold-out).  
  - Trains the final model used for SHAP analysis with all the data (including the hold-out).  
...
- `DRIVERS_10_net_ghg_exchange_budgets.ipynb`  
  - Converts fluxes to appropriate units for nitrogen and greenhouse gas (GHG) budgets.  
  - Calculates cumulative fluxes in appropriate unit.
  - Calculates net GHG exchange (NGHGE).
  - Calculates full GHG budgets (accounting for later C fluxes).
...
- `SOURCES_01_prep_inputs_FRAME.ipynb`  
  ...
  - Writes `N2O_sources_FRAME`, `N2O_samples_FRAME`, `N2O_fractionation_MR_FRAME`, and `N2O_fractionation_RM_FRAME` to `../data/processed/`.
